...
The Windows release decision is stricter than one runner invocation. Pass every raw result file
to `tools\Test-WindowsQualification.ps1` with the separately collected preview-window evidence.
That aggregator recomputes verdicts, rejects repeated labeled points, enforces the documented
risk-based OS/scenario/DPI/topology matrix, and emits a hash-addressed Markdown report. See

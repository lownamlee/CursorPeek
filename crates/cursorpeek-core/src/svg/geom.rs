...
fn append_cubic(output: &mut Vec<Point>, from: Point, first: Point, second: Point, to: Point) {
    let steps = curve_steps(
        from.distance_to(first) + first.distance_to(second) + second.distance_to(to),
    );
    for step in 1..=steps {
        let t = step as f32 / steps as f32;
        let inverse = 1.0 - t;
...
fn dash_polyline(points: &[Point], pattern: &[f32], offset: f32) -> Vec<Vec<Point>> {
    ...
    let mut lengths: Vec<f32> = pattern
        ...
        .collect();
    if lengths.is_empty() || pattern.iter().all(|value| !value.is_finite() || *value <= 0.0) {
...
mod tests {
    ...
    fn transforms_compose_invert_and_report_scale() {
        ...
        let inverse = combined.invert().expect("a scaled translation is invertible");
    ...
    fn malformed_path_data_fails_closed() {
        ...
        assert_eq!(parse_path("M0 0A5 5 0 2 0 1 1"), Err(SvgError::MalformedPath));
    }
    ...
    fn shape_helpers_produce_closed_geometry_and_bounds() {
        ...
        assert!(matches!(rounded[2], PathSeg::ArcTo { rx: 5.0, ry: 5.0, .. }));
    ...
    fn stroke_pieces_share_one_winding_direction() {
        let contours = flatten(&parse_path("M0 0 L10 0 L10 10").unwrap(), Transform::IDENTITY);

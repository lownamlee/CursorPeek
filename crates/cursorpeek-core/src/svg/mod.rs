...
//! worker. Callers fall back to the inert text preview on any error.
...
use crate::layout::{
    MAX_SOURCE_IMAGE_AXIS, MAX_SOURCE_IMAGE_PIXELS, MAX_VECTOR_FRAMES, MIN_VECTOR_FRAME_DELAY_MS,
    checked_vector_layout, fitted_vector_dimensions,
};
use geom::Transform;
...
pub fn render(source: &str) -> Result<VectorRender, SvgError> {
    ...
    let (width, height) = fitted_vector_dimensions(source_width, source_height, animated)
        .ok_or(SvgError::InvalidSize)?;
    let (frame_count, frame_delay_ms) = frame_plan(timeline);
    checked_vector_layout(width, height, frame_count).map_err(|_| SvgError::InvalidSize)?;
    ...
    if frames.iter().all(|frame| frame.iter().all(|byte| *byte == 0)) {
    ...
    fn fmt(&self, formatter: &mut fmt::Formatter<'_>) -> fmt::Result {
        match self {
            ...
            Self::EntityDeclaration => {
                write!(formatter, "the document declares entities or an internal subset")
            }
            Self::ActiveContent => {
                write!(formatter, "the document contains script or event-handler content")
            }
            Self::ExternalReference => {
                write!(formatter, "the document references a resource outside itself")
            }
            ...
            Self::NothingPainted => {
                write!(formatter, "the document paints nothing this renderer supports")
            }
        }
    }
}
...
mod tests {
    ...
    use crate::layout::{MAX_SOURCE_IMAGE_AXIS, MAX_VECTOR_UPSCALE};

    #[test]
    fn a_static_document_renders_one_upscaled_frame() {
        let rendered = render(
        ...
        assert_eq!(
            ...
            (16 * MAX_VECTOR_UPSCALE, 16 * MAX_VECTOR_UPSCALE)
        );
    ...
    fn an_animated_document_renders_distinct_bounded_frames() {
        ...
        assert_ne!(
            rendered.frames[0], rendered.frames[rendered.frames.len() - 1],
